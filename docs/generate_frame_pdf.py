#!/usr/bin/env python3
"""
Generate PDF from FRAME_User_Manual.md
"""
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'FRAME Command User Manual', 0, 1, 'C')
        self.ln(5)
...
# Box-drawing and other Unicode replacements for latin-1 encoding
...
def sanitize(text):
    ...
    for uchar, replacement in UNICODE_REPLACEMENTS.items():
...
def add_table(pdf, headers, rows):
    ...
    if num_cols == 2:
        max_first = max(len(headers[0]), max((len(r[0]) for r in rows), default=0))
        max_second = max(len(headers[1]), max((len(r[1]) for r in rows), default=0))
        total = max_first + max_second
        if total > 0:
            first_w = max(40, min(120, int(page_width * max_first / total)))
            col_widths = [first_w, page_width - first_w]
        else:
            col_widths = [95, 95]
    elif num_cols == 3:
        col_widths = [50, 50, 90]
...
def process_markdown(pdf, content):
    ...
    while i < len(lines):
        ...
        # Bold text as mini headers
            ...
            pdf.multi_cell(0, 5, text)
        # Bold text followed by content (e.g. "**Note:** some text")
        elif line.strip().startswith('**') and '**' in line.strip()[2:]:
            pdf.set_font('Helvetica', '', 10)
            text = sanitize(line.strip().replace('**', '').replace('`', '"'))
            pdf.multi_cell(0, 5, text)
        # List items
...
def main():
    ...
    with open('FRAME_User_Manual.md', 'r', encoding='utf-8') as f:
        content = f.read()
    process_markdown(pdf, content)

    output_file = 'FRAME_User_Manual.pdf'
