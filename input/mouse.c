...
{
  if (!(MOUSE_CLOCK || Option.MOUSE_CLOCK))
    return;
    ...
    case PS2STOP:
      ...
      { // PS2DAT == 1
        ...
        if (runmode)
        {
          mouse[bno++] = Code;
          if (!(mouse[0] & 0x08))
            bno = 0; // bit 3 must be set in first byte
          if (bno == (mouseID == 3 ? 4 : 3))
          {
            ...
            if (nunstruct[2].ax < 0)
              nunstruct[2].ax = 0;
            if (nunstruct[2].ax >= maxW)
              nunstruct[2].ax = maxW - 1;
            nunstruct[2].ay -= mouse[2];
            mouseupdated = 1;
            if (nunstruct[2].ay < 0)
              nunstruct[2].ay = 0;
            if (nunstruct[2].ay >= maxH)
              nunstruct[2].ay = maxH - 1;
            nunstruct[2].L = mouse[0] & 0b1;
            nunstruct[2].R = (mouse[0] & 0b10) >> 1;
            nunstruct[2].C = (mouse[0] & 0b100) >> 2;
            if (nunstruct[2].type > 1000)
              nunstruct[2].Z = 0;
...
void cmd_mouse(void)
{
  ...
  else if ((tp = checkstring(cmdline, (unsigned char *)"CLOSE")))
  {
    ...
    if (!mouse0)
      error("Not open");
    if (Option.MOUSE_CLOCK)
      error("Option MOUSE set - close invalid");
    n = getint(argv[0], 2, 2);
    mouse0close();
  }
