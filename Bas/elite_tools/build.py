...
def main():
    ...
    if "--full" not in sys.argv:
        text = ("' Elite for the PicoMite, built by elite_tools/build.py from src/*.bas.\n"
                "' Comments and indentation are stripped to fit program memory - read src/.\n") + strip(text)
    open(OUT, "w", newline="\n", encoding="utf-8").write(text)
    n = text.count("\n")
    print("built %s: %d lines, %d bytes (%d before stripping) from %s + ships.bas"
          % (OUT, n, len(text), full, ", ".join(names)))
    if len(text) > PROGRAM_LIMIT:
        print("  ** %d bytes is over the %d the PicoMite will hold **" % (len(text), PROGRAM_LIMIT))
    if problems:
