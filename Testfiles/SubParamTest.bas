...
Sub TestPointStruct p As TPoint
  AssertEqual p.x, 50, "Struct param received (x)"
  AssertEqual p.y, 75, "Struct param received (y)"
  p.x = 999
  p.y = 888
End Sub

Sub TestPointArray p() As TPoint
  AssertEqual p(3).x, 30, "Struct array element received (x)"
  AssertEqual p(3).y, 60, "Struct array element received (y)"
  p(2).x = 777
  p(2).y = 666
End Sub
